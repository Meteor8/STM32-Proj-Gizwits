...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagled0)
    {
        info->event[info->num] = EVENT_led0;
        info->num++;
        dataPoints->valueled0 = gizStandardDecompressionValue(led0_BYTEOFFSET,led0_BITOFFSET,led0_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagled1)
    {
        ...
        info->num++;
        dataPoints->valueled1 = gizX2Y(led1_RATIO,  led1_ADDITION, issuedData->attrVals.valueled1);
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueled0 != cur->valueled0)
    {
        GIZWITS_LOG("valueled0 Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(led0_BYTEOFFSET,led0_BITOFFSET,led0_LEN,(uint8_t *)devStatusPtr,dataPoints->valueled0);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueled1 = gizY2X(led1_RATIO,  led1_ADDITION, dataPoints->valueled1);
